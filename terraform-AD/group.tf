...
resource "azuread_group_member" "department" {
  for_each = { for user in azuread_user.users : user.value.first_name => user if user.department == "DevOps"}
...
resource "azuread_group_member" "Software" {
    for_each = { for user in azuread_user.users : user.value.first_name => user if user.job-title == "Software Engineer"}
...
resource "azuread_group_member" "Manager" {
  for_each = { for user in azuread_user.users : user.value.first_name => user if user.job-title == "Manager"}
